...
def trace_last(n: int=typer.Argument(10, help='Number of spans to show'), filter_attr: str | None=typer.Option(None, '--filter', '-f', help='Filter by attribute, e.g. "role=Researcher"'), file_path: Path | None=typer.Option(None, '--file', help='Read from JSONL file instead of buffer'), verbose: bool=typer.Option(False, '--verbose', '-v', help='Show full attributes')) -> None:
